...
let inputCharTyped fs subDirResults progress cancelInput char model = asyncSeqResult {
    ...
    | Some (Input (Find _)) ->
        if Path.InvalidNameChars |> String.contains (string char) then
            cancelInput()
        else if KeyBinding.getKeysString (Cursor FindNext) |> Seq.toList = [char] then
            cancelInput ()
    ...
    | Some (Confirm confirmType) ->
        ...
        | 'y' ->
            ...
            | Overwrite (putType, srcDestPairs) ->
                ...
                let! model = ItemActionCommands.putInLocation fs progress false true putType itemRefs model
                yield { model with Config = { model.Config with YankRegister = None } }
            | Delete ->
                yield! ItemActionCommands.delete fs progress model.ActionItems model
...
let submitInput fs os model = asyncSeqResult {
    ...
    | Some (Input NewFile) ->
        let model = { model with InputMode = None }
        yield model
        yield! ItemActionCommands.create fs File model.InputText model
    | Some (Input NewFolder) ->
        let model = { model with InputMode = None }
        yield model
        yield! ItemActionCommands.create fs Folder model.InputText model
    | Some (Input (Rename _)) ->
        let model = { model with InputMode = None }
        yield model
        yield! ItemActionCommands.rename fs model.CursorItem model.InputText model
    | _ -> ()
}
...
let cancelInput model =
    ...
    |> match model.InputMode with
        | Some (Input Search) -> NavigationCommands.clearSearch
        | _ -> id
...
let windowActivated fsReader subDirResults progress model = asyncSeqResult {
    if model.Config.Window.RefreshOnActivate then
        if model.IsSearchingSubFolders then
            yield model |> NavigationCommands.refreshDirectory fsReader
        else
            yield! model |> NavigationCommands.refreshOrResearch fsReader subDirResults progress
}

let isEventHandledInBackground evt =
    match evt with
    | ConfigFileChanged _
    | HistoryFileChanged _
    | PageSizeChanged _
    | WindowLocationChanged _
    | WindowSizeChanged _
    | WindowMaximizedChanged _
    | WindowActivated ->
        true
    | _ ->
        false
...
type Controller(
    cursorCommandHandler: CursorCommands.Handler,
    navigationCommandHandler: NavigationCommands.Handler,
    itemActionCommandHandler: ItemActionCommands.Handler,
    windowCommandHandler: WindowCommands.Handler,
    fs: IFileSystem,
    os: IOperatingSystem,
    ...
    let handleCommand (command: MainCommand) =
        match command with
        | Cursor command -> cursorCommandHandler.Handle command
        | Navigation command -> navigationCommandHandler.Handle command
        | ItemAction command -> itemActionCommandHandler.Handle command
        | Window command -> windowCommandHandler.Handle command
    ...
    let dispatcher evt =
        let handler =
            ...
            // TODO: call handler members instead of module functions
            | LocationInputChanged -> Async (NavigationCommands.suggestPaths fs)
            | LocationInputSubmit (path, handler) -> SyncResult (NavigationCommands.openInputPath fs os path handler)
            | LocationInputCancel -> Sync (fun m -> { m with LocationInput = m.LocationFormatted })
            | DeletePathSuggestion path -> Sync (NavigationCommands.deletePathSuggestion path)
            // TODO: delegate input events to command handlers
            ...
            | SubDirectoryResults items -> Sync (NavigationCommands.addSubDirResults items)
            | UpdateDropInPutType (paths, event) -> Sync (ItemActionCommands.updateDropInPutType paths event)
            | DropIn (paths, event) -> AsyncResult (ItemActionCommands.dropIn fs progress paths event)
            | DropOut event -> Sync (ItemActionCommands.dropOut fs event)
        ...
        if isEventHandledInBackground evt then
            handler
        else
            match handler with
            | Sync handler ->
