...
let ``toggleHidden and redo handles partial success by updating undo and redo and setting error message`` isRedo =
    ...
    let model =
        ...
        |> applyIf isRedo (pushRedo (ToggleHidden (true, actionItems, false)))
    let testFunc =
        if isRedo
        then Undo.redo fs progress
        else Attributes.toggleHidden fs progress
    ...
    let expectedError = MainStatus.ToggleHiddenError (true, [errorItem.Path, ex], actionItems.Length)
    let expectedUndo = ToggleHidden (true, actionItems |> List.except [errorItem], false)
    let expected =
        { model with
            Directory = expectedItems
            Items = expectedItems |> List.filter (not << _.IsHidden)
            SelectedItems = [] // selected list isn't set for only one item since cursor is sufficient
...
[<Test>]
let ``undo toggleHidden handles partial success by updating undo and redo and setting error message`` () =
    let fs = FakeFileSystem [
        fileWith (hide true) "file1"
        fileWith (hide true) "file2"
        fileWith (hide true) "file3"
    ...
    let model =
        ...
        |> pushUndo (ToggleHidden (true, actionItems, false))
    ...
    let expectedSucceeded = [
        expectedItems.[0]
        expectedItems.[2]
    ]
    let expectedError = MainStatus.ToggleHiddenError (false, [errorItem.Path, ex], actionItems.Length)
    let expectedRedo = ToggleHidden (true, expectedSucceeded, false)
    let expected =
        { model with
            Directory = expectedItems
            Items = expectedItems |> List.filter (not << _.IsHidden)
            SelectedItems = expectedSucceeded
