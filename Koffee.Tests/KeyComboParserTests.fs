...
type KeyComboParserTests() =
    ...
    [<Test>]
    member x.``Given a lowercase letter, returns matching key.``() =
        assertParseSeq ['a'..'z'] Key.A ModifierKeys.None

    [<Test>]
    member x.``Given an uppercase letter, returns matching key with shift.``() =
        assertParseSeq ['A'..'Z'] Key.A ModifierKeys.Shift

    [<Test>]
    member x.``Given a number, returns matching key.``() =
    ...
    member x.``Given a symbol, returns matching key.`` input (expectedKey: Key) expectShift =
    ...
    member x.``Given a key name, returns matching key.`` input (expectedKey: Key) =
        assertParsesToSingle input (ModifierKeys.None, expectedKey)

    [<TestCase("<s>", Key.S, false)>]
    [<TestCase("<S>", Key.S, true)>]
    [<TestCase("<!>", Key.D1, true)>]
    member x.``Given a key in brackets, returns matching key and modifier.`` input (expectedKey: Key) expectShift =
        let expectedMod = if expectShift then ModifierKeys.Shift else ModifierKeys.None
        assertParsesToSingle input (expectedMod, expectedKey)
    ...
    member x.``Given a key name and shift modifer character, returns matching key with shift.`` input (expectedKey: Key) =
    ...
    member x.``Given a modifier character, returns matching key with the modifer.`` inputKey (expectedKey: Key) expectShift =
    ...
    member x.``Given multiple modifer characters, returns matching key with modifiers.`` input (expectedMod: ModifierKeys) (expectedKey: Key) =
        assertParsesToSingle input (expectedMod, expectedKey)

    [<Test>]
    member x.``Given 3 letters, returns a sequence of those keys.``() =
        parseKey "abc"
        |> should equal [(ModifierKeys.None, Key.A); (ModifierKeys.None, Key.B); (ModifierKeys.None, Key.C)]

    [<Test>]
    member x.``Given 3 chords, returns a sequence of those chords.``() =
        parseKey "<c-a><a-b>C"
        |> should equal [(ModifierKeys.Control, Key.A); (ModifierKeys.Alt, Key.B); (ModifierKeys.Shift, Key.C)]

    [<Test>]
    member x.``Default key bindings, which uses parsing, does not throw an error.``() =
    ...
    [<TestCase("<-tab>")>]
    [<TestCase("<s-tab>>")>]
    member x.``Given invalid input, returns None.`` input =
