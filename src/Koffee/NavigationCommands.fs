...
let startSearch (model: MainModel) =
...
let private startPrompt prompt (model: MainModel) =
    let inputMode =
...
let handleCommand fs subDirResults progress (command: NavigationCommand) =
    match command with
    ...
    | StartSearch -> Sync startSearch
    | RepeatPreviousSearch -> Async (repeatSearch fs subDirResults progress)
    | PromptGoToBookmark -> Sync (startPrompt GoToBookmark)
    | PromptGoToSavedSearch -> Sync (startPrompt GoToSavedSearch)
    | PromptSetBookmarkOrSavedSearch -> Sync (startPrompt SetBookmark)
