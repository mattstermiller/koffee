...
type Controller(
    ...
    let dispatcher evt =
        let handler =
            ...
            | UpdateDropInPutType (paths, event) -> Sync (ItemActionCommands.Put.updateDropInPutType paths event)
            | DropIn (paths, event) -> AsyncResult (ItemActionCommands.Put.dropIn fs progress paths event)
            | DropOut event -> Sync (ItemActionCommands.Put.dropOut fs event)
            ...
            | Background WindowActivated -> AsyncResult (windowActivated fs subDirResults progress)
